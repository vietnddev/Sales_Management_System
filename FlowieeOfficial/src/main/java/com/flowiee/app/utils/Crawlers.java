...
public class Crawlers {
    ...
    private Document detailPage;
    private ArrayList<String> list_URL_Detail = new ArrayList<>();
    private ArrayList<Product> listFullInformation = new ArrayList<Product>();
    ...
    public List<Product> crawlersDataFromJunoShop(){
        List<Product> list = new ArrayList<>();

        getListName();
        for (int i = 0; i < getListName().size(); i++){
            list.add(new Product("code", "Quần áo", getListName().get(i), "color", 0.0d,
                    "size", "date", 0, 0, "describes", true,
                    0));
        }

        getListPrice();
        for (int i = 0; i < list.size(); i++) {
            list.get(i).setPrice(getListPrice().get(i));
    ...
    private ArrayList<String> getListCode() {
    ...
    public ArrayList<Double> getListPrice() {
        ...
        try {
            ...
            for (Element price : prices) {
                list.add(Double.parseDouble(price.text()));
            }
        } catch (Exception e) {
            System.out.println(e.getCause());
        }
...
//                    }
    ...
    ArrayList<Double> listPrice = getListPrice();
...
//            product.setPrice(listPrice.get(i));
