...
import com.flowiee.app.services.MailService;
...
public class LogController {
    ...
    public ResponseEntity<?> exportLog(){
        String username = accountService.getUserName();
        String nameSheet = "SystemLog" + "_" + DatetimeUtil.now("yyyyMMdd HHmmss");
        String fileName = "SystemLog" + "_" + DatetimeUtil.now("yyyyMMdd HHmmss");
        if (username != null && !username.isEmpty()){
            try {
                ...
                for (int i = 1; i < list.size(); i++) {
                    ...
                    row.createCell(4).setCellValue(list.get(i).getCreated());
