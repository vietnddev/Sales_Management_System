...
public class ProductController extends BaseController {
    ...
    @Autowired
    public ProductController(ProductService productService, ProductHistoryService productHistoryService, FileStorageService fileStorageService) {
    ...
    public ApiResponse<List<ProductDTO>> findProducts(@RequestParam(value = "pageSize", required = false) Integer pageSize,
        ...
        try {
            ...
            Page<ProductDTO> productPage = productService.findAllProducts(pageSize, pageNum - 1, txtSearch, null, null, null);
            return ApiResponse.ok(productPage.getContent(), pageNum, pageSize, productPage.getTotalPages(), productPage.getTotalElements());
    ...
    public ApiResponse<List<ProductDetail>> findProductVariants() {
        if (!super.vldModuleProduct.readProduct(true)) {
            return null;
        }
        try {
            List<ProductDetail> result = productService.findAllProductVariants();
    ...
    public ApiResponse<ProductDetail> findDetailProductVariant(@PathVariable("id") Integer productVariantId) {
        if (!super.vldModuleProduct.readProduct(true)) {
            return null;
        }
        try {
    ...
    public ApiResponse<ProductDetail> updateProductVariant(@RequestBody ProductDetail productDetail, @PathVariable("id") Integer productVariantId) {
        if (!super.vldModuleProduct.updateProduct(true)) {
            return null;
        }
        try {
            return ApiResponse.ok(productService.updateProductVariant(productDetail, productVariantId));
        } catch (RuntimeException ex) {
            throw new AppException(String.format(MessageUtils.UPDATE_ERROR_OCCURRED, "product"));
        }
    }
    ...
    public ApiResponse<String> updatePrice(@PathVariable("productVariantId") Integer productVariantId,
        ...
        try {
            ...
            return ApiResponse.ok(productService.updatePrice(productVariantId, originalPrice, discountPrice));
        } catch (RuntimeException ex) {
    ...
    public ApiResponse<List<ProductHistory>> getHistoryPriceOfProductDetail(@PathVariable("Id") Integer productVariantId) {
        try {
            ...
            if (ObjectUtils.isEmpty(productService.findProductVariantById(productVariantId))) {
                throw new BadRequestException();
            }
    ...
    public ApiResponse<List<FileStorage>> viewGallery() {
        try {
            ...
            return ApiResponse.ok(fileStorageService.getAllImageSanPham(MODULE.PRODUCT.name()));
        } catch (RuntimeException ex) {
            throw new AppException(String.format(MessageUtils.SEARCH_ERROR_OCCURRED, "gallery"));
        }
    }
