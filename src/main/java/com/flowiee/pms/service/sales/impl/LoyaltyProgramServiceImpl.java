...
public class LoyaltyProgramServiceImpl extends BaseService implements LoyaltyProgramService {
    ...
    private BigDecimal getPoints(BigDecimal orderValue, LoyaltyProgram program) {
        ...
        for (LoyaltyRule rule : ruleList) {
            ...
            if (orderValue.compareTo(rule.getMinOrderValue()) >= 0
                    && (rule.getMaxOrderValue() == null || orderValue.compareTo(rule.getMaxOrderValue()) < 0)) {
                return orderValue.multiply(rule.getPointConversionRate()).setScale(0, RoundingMode.HALF_UP);
            }
        }
